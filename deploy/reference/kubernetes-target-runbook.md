...
В cluster размещаются Edge, Director, Agent Gateway, PVC и migration Job.
...
3. `20-workloads.json` — Edge, Director и Gateway Deployments;
4. `render-evidence.json` — apply order, counts и SHA-256 каждого bundle.
...
2. Выполнить три server-side dry-run.
...
Workloads нельзя применять параллельно с migration Job. Failed Job, unbound PVC,
Pod Security rejection, image digest mismatch, failed readiness или NetworkPolicy
connectivity failure являются stop-ship.
