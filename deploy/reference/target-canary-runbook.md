...
- Director -> Gateway: правильные client certificate и Bearer доходят до
  доменного `404 not_found`, а отсутствующий или неверный Bearer и отсутствие
  client certificate отклоняются;
- Gateway -> Director: правильные client certificate и Bearer доходят до
  `403 capability_invalid`, а отсутствующий или неверный Bearer и отсутствие
  client certificate отклоняются;
...
монтирует CA/cert/key, service bearer files и session token read-only и удаляется
...
- private keys, bearer/session files имеют mode `0400`, `0440`, `0600` или
...
6. Четыре `registry_updates` перенесены без изменения статуса и с исходным
...
любой internal endpoint принимает один фактор вместо mTLS+Bearer, правильная
