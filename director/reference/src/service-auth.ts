...
export class WorkloadIdentityAuthenticator implements ServiceAuthenticator {
  ...
  authenticate(input: ServiceAuthInput): void {
    ...
    if (match?.[1] === undefined) {
      throw new DirectorProtocolError(401, 'unauthorized_service', 'Workload token is required.');
    }
    try {
      this.verifier.verify(match[1]);
    } catch {
      throw new DirectorProtocolError(401, 'unauthorized_service', 'Workload token is invalid.');
    }
