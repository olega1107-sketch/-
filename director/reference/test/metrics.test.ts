...
describe('Director Prometheus metrics', () => {
  it('exports bounded route labels and cumulative duration buckets', () => {
    ...
    metrics.recordDependencyReadiness(true, false);
    ...
    expect(rendered).toContain('dirizhor_postgres_up{service="director"} 1');
    expect(rendered).toContain('dirizhor_document_store_up{service="director"} 0');
    expect(rendered).toContain('dirizhor_audit_write_failures_total{service="director"} 1');
