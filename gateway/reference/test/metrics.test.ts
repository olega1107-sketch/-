...
describe('Gateway Prometheus metrics', () => {
  it('exports bounded route labels and cumulative duration buckets', () => {
    ...
    expect(rendered).toContain('dirizhor_gateway_queue_pending{service="gateway"} 2');
    expect(rendered).toContain('dirizhor_gateway_queue_oldest_seconds{service="gateway"} 42.5');
    expect(rendered).toContain('dirizhor_gateway_queue_scan_failures_total{service="gateway"} 1');
